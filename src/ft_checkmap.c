...
void	ft_checkFirstAndLastLine(st_map *lst, int i)
{
	int	k;
...
void	ft_check_other_wall(st_map *lst)
{
	int	i;
	int	k;
...
void	ft_init_parametrs(st_map *lst)
{
	lst->sum_wall = 0;
	lst->sum_exit = 0;
	lst->sum_goal = 0;
...
void	ft_allowed_symbols(char c, st_map *lst)
{
	if (c == '1')
		lst->sum_wall++;
	if (c == 'C')
...
void	ft_check_all_other(st_map *lst, int i, int k)
{
	while (i != lst->height - 1)
	{
		while (k != lst->width - 1)
